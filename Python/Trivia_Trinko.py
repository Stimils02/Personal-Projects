...
def board():
    ...
    TrinkoImage = Image(Point(250, 50), gif_filename)
...
def control():
    ...
    image = Image(Point(250, 50), gif_filename)
    image.draw(win)
...
def drop(num, win, pin, score, tokens):
    ...
    bin_boundary = [(6, 74), (76, 144), (146, 214), (216, 284),
                    (286, 354), (356, 424), (426, 494), (496, 566)]
...
def makeQuiz():
    ...
    TrinkoImage = Image(Point(200, 360), gif_filename)
    TrinkoImage.draw(win)
...
def pickQs():
    ...
    for i in range(0, len(question_list)):
        ...
        question_dict[question_list[i][0]] = (question_list[i][1], question_list[i][2],
                                              question_list[i][3], question_list[i][4], question_list[i][5].replace('\n', ''))
...
def result(score_lst, entry_lst):
    ...
    message = Text(
        Point(250, 70), 'The winner of Trivia Trinko gameplay is...')
    message.setSize(13)
    message.setFace('courier')
    message.setStyle('bold')
    ...
    msg = Text(Point(250, 195),
               'The loser and the dumber of the two players is...')
    msg.setSize(13)
    msg.setFace('courier')
    msg.setStyle('bold')
    txt_lst.append(msg)
